...
Multiple simplifying assumptions were required of $g_\theta (\Lambda)$ to make this computationally efficient.
The first simplification was to restrict the filter to be a polynomial of the eigenvalues, which allows us to avoid explicitly computing the eigenvectors and instead use powers of the Laplacian.
This was further simplified by using chebyshev polynomials to approximate the polynomial filter.
Finally, the current standard GCN layer \cite{kipf2017semi} uses a first order approximation of the chebyshev polynomials and a normalized form of the Laplacian to create individual convolutinal layers.
