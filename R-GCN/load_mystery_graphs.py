...
NUM_COARSE_RELATIONS = len(COARSE_LABELS)  # 8
...
def load_mystery_graphs(
    ...
    with 8 coarse relation types (v3 schema — see Stage 2 comments above).
    ...
    intermediate_counts = {}   # intermediate form → count (across all edges)
    ...
    for fname in json_files:
        ...
        for edge in data.get("edges", []):
            ...
            edge_bucket.append((
                local_to_global[src_local],
                local_to_global[dst_local],
                coarse_id,
            ))
    ...
    print(f"Relation types : {NUM_COARSE_RELATIONS}  (coarsened from "
          f"{len(intermediate_counts)} intermediate forms, residual → social)")
    ...
    for rel_id, label in COARSE_LABELS.items():
        count = coarse_counts[rel_id]
        total = len(train_edges) + len(val_edges) + len(test_edges)
        pct   = 100.0 * count / total if total else 0
        print(f"  {rel_id}  {label:<18}  {count:>6,}  ({pct:5.1f}%)")
